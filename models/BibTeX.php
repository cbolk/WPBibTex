...
class Structures_BibTex
{
     ...
     * @access public
     * @var integer
     * if entry larger than it, it's drop entry.
     * if 0, then unlimited.
     */
    var $maxEntryLength;
    ...
    function Structures_BibTex($options = array())
    {
        ...
        $this->authorstring = 'FIRST MIDDLE LAST';
    ...
    function _parseEntry($entry)
    {
        ...
        $ret = array();
        if ($this->maxEntryLength and $this->maxEntryLength < strlen($entry))
        {
            $this->_generateWarning('WARNING_TOO_BIG_ENTRY', '', $entry.'}');
            return $ret;
        }

//    $text = preg_replace('/([^\x20-\x7F]|\*)+/', '%', $text); //@TODO it's not working with UTF-16
        if (preg_match('/([^\r\n\t\x20-\x7F]|\*)+/', $entry))
        {
            $this->_generateWarning('WARNING_ILLEGAL_CHARACTER', '', $entry.'}');
            return $ret;
        }
        ...
        if ('@string' ==  strtolower(substr($entry, 0, 7))) {
            ...
            //Parsing all fields
            while (($position = strrpos($entry,'=')) !== false) {
    ...
    function _extractAuthors($entry) {
        ...
        for ($i = 0; $i < sizeof($authorarray); $i++) {
            ...
            if (strpos($author, ',') === false) {
            	/* The first version of how an author could be written (First [Middle] Last)
             	   has no commas in it*/
                $tmparray = array();
                ...
                if (1 == $size) { //There is only a last
                    ...
                    $first = $tmparray[0];
                    $last  = $tmparray[1];
				} else { /* (3 == $size) { There is also the middle name */
                    $first = $tmparray[0];
                    $middle  = $tmparray[1];
                    $last  = $tmparray[2];
                } 
            } else { //Version 2 and 3 (Last, First [Middle])
                $tmparray     = array();
                $tmparray     = explode(',', $author);
                //The first entry must contain last
				$last = $tmparray[0];
                $firstmiddle = array();
                $firstmiddle = explode(' ', $tmparray[1]);
                $size         = sizeof($firstmiddle);
                if (1==$size) { //Only one entry->got to be the first
                    $first = $firstmiddle[0];
                } else {
                    $first = $firstmiddle[0];
                    $first = $firstmiddle[1];
            ...
            $authorarray[$i] = array('first'=>trim($first), 'middle'=>trim($middle), 'last'=>trim($last));
        }
        return $authorarray;
    }
    ...
    function _formatAuthor($array)
    {
        ...
        if (!array_key_exists('middle', $array)) {
            $array['middle'] = '';
        } else {
            $array['middle'] = trim($array['middle']);
        }
        ...
        $ret = str_replace("MIDDLE", $array['middle'], $ret);
